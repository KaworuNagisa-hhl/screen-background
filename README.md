...
```bash
ohpm install @kaworunagisa_hhl/screen-background
```
...
```ts
import { SwiftUIScreenBackground } from '@kaworunagisa_hhl/screen-background'
...
完整最小示例见 `example/SwiftUIScreenBackgroundUsage.ets`。该示例演示了全屏背景与前景内容叠加，适合作为页面底色或工作台背景的接入模板。
